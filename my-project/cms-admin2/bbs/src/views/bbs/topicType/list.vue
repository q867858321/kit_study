      ...
      <form class="form-horizontal no-margin" >
        <el-table style="width: 100%" :data="topicTypeList" @selection-change="checkIds">
          ...
          <el-table-column label="操作"  :width="200" align="center">
            <template slot-scope="scope">
              <a href="javascript:void(0)" class="t-edit iconfont bbs-edit" title="编辑" @click="editTopicType(scope.row.id)"></a>
              <a href="javascript:void(0)" class="t-del iconfont bbs-delete" title="删除" @click="deleteTopicType(scope.row.id)"></a>
            </template>
          </el-table-column>
        </el-table>
        <!--收缩隐藏-->
...
<script>
  ...
  export default {
    ...
    methods: {
      getTopicTypeList() { // 获取主题信息列表
        let params = this.params;
        params.siteId = this.siteId;
        fetch.getTopicTypeList(params)
          .then(res => {
            if (res.code == 1) {
      ...
      deleteTopicType(id) { // 删除主题
        this.ids = id;
        this.$confirm('您确定要删除主题吗?', '警告', { type: 'warning' })
          .then(mes => {
            fetch.deleteTopicTypeInfo({ids: this.ids})
              .then(res => {
                this.loading = false;
                if (res.code == 1) {
