<template>
  <div class="forum-module" v-loading.body="loading">
    ...
    <el-form :inline="true" class="demo-form-inline" style="margin:20px 20px 0;">
      ...
      <el-button type="text">
        <router-link :to="{path:'topic-edit',query:{type:'add'}}">新增</router-link>
      </el-button>
    </el-form>
        ...
        <tbody class="first-table">
          <tr class="parent-module" v-for="(item,index) in topicList" :key="index">
            ...
            <td style="width:300px;">
              ...
              <span class="top" v-if="item.isTop">顶</span>
              <span class="top" v-if="item.isPrime">精</span>
              <span class="top" v-if="item.status==-1">蔽</span>
              <span class="top" v-if="item.status==1">锁</span>
            </td>
            ...
            <td style="width:250px;" class="btn_box">
              <el-button type="text" @click="topicOperate('toPrime',item.id,index)">精</el-button>
              <el-button type="text" @click="topicOperate('toTop',item.id,index)">置顶</el-button>
              <el-button type="text" @click="topicOperate('toShield',item.id,index)">屏蔽</el-button>
              <el-button type="text">
                <router-link :to="{path:'topic-edit',query:{'id':item.id,type:'update'}}">详情</router-link>
              </el-button>
  ...
  methods: {
    getTopicList() {
      ...
      fetch
        .getTopicList(params)
        .then(res => {
          console.log("topic list response", res);
          if (res.code == 1) {
    ...
    topicOperate(method, id, index) {
      if (method == "toPrime") {
        this.toPrime(id, index);
      } else if (method == "toTop") {
        this.toTop(id, index);
      } else if (method == "toShield") {
        this.toShield(id, index);
      }
    },
    toTop(id, index) {
      let isTop = this.topicList[index].isTop;
      let topLevel = 0;
      if (!isTop) {
        topLevel = 1;
      }
      ...
      fetch.topTopic(params).then(res => {
        console.log("topTop res", res);
        if (res.code == 1) {
          this.topicList[index].isTop = !isTop;
          this.topicList[index].topLevel = topLevel;
        } else {
          this.$message.error(res.msg);
        }
      });
    ...
    toPrime(id, index) {
      let isPrime = this.topicList[index].isPrime;
      let primeLevel = 0;
      if (!isPrime) {
        primeLevel = 1;
      }
      ...
      fetch.primeTopic(params).then(res => {
        if (res.code == 1) {
          this.topicList[index].isPrime = !isPrime;
          this.topicList[index].primeLevel = primeLevel;
        } else {
          this.$message.error(res.msg);
        }
      });
    ...
    toShield(id, index) {
      ...
      let status = this.topicList[index].status;
      fetch.primeTopic(params).then(res => {
        console.log("shield", res);
        if (status == 0) {
          this.topicList[index].status = -1;
        } else {
          this.topicList[index].status = 0;
        }
      });
    },
    //弹出框的操作
