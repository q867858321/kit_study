<template>
  <section class="v_p-edit">
    <div class="stik__top">
      <span class="link" @click="$router.go(-1)">{{name}}(项目)</span>
...
export default {
  ...
  methods: {
    ...
    saveResult() {
      ...
      if (deletedCompIds) {
        assemblyDeleteApi({ ids: deletedCompIds })
          .then(res => {
            this.$message({
              message: "删除成功",
              type: "success"
