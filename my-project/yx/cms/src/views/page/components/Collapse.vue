  ...
  <section class="v_p_c-collapse">
    <el-collapse v-if="editSpecComponent" :value="Object.keys(attributes)">
      <el-form v-if="activeNames.length" ref="dataForm" :model="attributes">
        <el-collapse-item
          v-for="(attr, index) in getSpecCompAttr"
          :title="attr.name"
          :name="attr.eName"
          :key="index"
        >
          <div v-if="attr.type=='radio'">
            <el-radio-group v-model="attributes[attr.eName]">
              <el-radio
                v-for="(option,index) in attr.attrOptionList"
                :label="option[0]"
                :key="index"
              >{{option[1]}}</el-radio>
            </el-radio-group>
...
export default {
  ...
  computed: {
    ...
    contents() {
      ...
      return (
        this.editSpecComponent.contents &&
        this.editSpecComponent.contents.length
      );
    },
    activeNames() {
      this.attributes = Object.assign({}, this.editSpecComponent.attributes, {
        source: 0
      });
