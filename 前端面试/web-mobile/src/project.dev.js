    ...
    cc.Class({
      ...
      start: function start() {
        ...
        window.addEventListener("message", function (event) {
          ...
          if (extra == NEW_GAME) _me.init();
          else if (extra == HAMMER_GET_HAMMER) {
      ...
      init: function init() {
        ...
        if (this.blocks)
          for (var i = 0; i < this.blocks.length; i++)
            for (var j = 0; j < this.blocks[i].length; j++) null != this.blocks[i][j] && this.blocks[i][j].destroy();
        this.data = [];
        this.blocks = [];
      ...
      findBlockTouched: function findBlockTouched(event) {
        ...
        for (var i = 0; i < this.poisitions.length; i++)
          for (var j = 0; j < this.poisitions[i].length; j++) {
      ...
      updateHighestScore: function updateHighestScore() {
        ...
        if (null == high)
          ...
          } else this.highestScoreLabel.string = "0";
        else if (null != this.score)
          if (high < this.score) {
            cc.sys.localStorage.setItem(HIGHEST_SCORE, this.score);
            this.highestScoreLabel.string = this.score;
          } else this.highestScoreLabel.string = high;
        else this.highestScoreLabel.string = high;
      },
      ...
      getEmptyLocations: function getEmptyLocations() {
        var locations = [];
        for (var i = 0; i < this.blocks.length; i++)
          for (var j = 0; j < this.blocks[i].length; j++) null == this.blocks[i][j] && locations.push({
            x: i,
            y: j
          });
        return locations;
